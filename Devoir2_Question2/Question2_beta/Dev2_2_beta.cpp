...
public:
	...
	void inserer(unType* &donnee)
	{
		...
		delete donnee;		// Il faut qu'à la fin, le pointeur Queue  indique le dernier noeud.			// Le dernier noeud a comme particularité que sont champ suivant contient l'adresse NULL.
...
void trouver_la_mediane(long T[], long taille )
{
	double position = (taille + 2) / 2;
	long test = position;
	if (test == position) { cout << "1 mediane incontestee " << T[test] << endl;  }
	else 
	{
		double resultat = T[test] + ((T[test + 1] - T[test])*(position - test));
		cout << "Mediane determinee : " << resultat << endl;
	}
...
int main()
{
	...
	long Attente [1000]; long Transactions[1000];
	...
	unClient  *temp = new unClient;
	temp->dureeTransaction = obtenirDureeTransaction(duree_min, duree_max); temp->tempArrivee = 0;
	file.inserer(temp); 
    ...
    while (heure_horloge <= 480  || (file.estVide() == false ))
	{
		...
		while (curseur->information.dureeTransaction != 0 ) 
		{
			...
			{
				unClient *nouveau = new unClient; 
				nouveau->dureeTransaction = obtenirDureeTransaction(duree_min, duree_max); nouveau->tempArrivee = heure_horloge;
				file.inserer(nouveau);
			}
			curseur->information.dureeTransaction--; heure_horloge++; // reduire le temps de transaction 		// incrementer le temps sur l'horloge 
		}
		...
		enlevement += 1;
		file.enlever(curseur->information); 
	...
	cout << endl << "|||||||||||||||Duree moyenne d'attente : " << duree_moyenne<< endl;
	cout << endl << "Plus courte duree d'attente : " << duree_min <<endl;
	cout << endl << "Plus grande duree d'attente : " << duree_max << endl;
	cout << endl << "|||||||||||||||Duree moyenne de transaction : " << duree_moyenne_de_transaction<< endl;
	...
	cout << "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||" << endl;
	trouver_la_mediane(Attente, compteur);
